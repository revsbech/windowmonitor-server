...
func main() {
	...
	http.HandleFunc("/listen", getListenHandler())
	http.HandleFunc("/stats", statsHandler)
	rand.Seed(time.Now().UnixNano())

		// Do a lopp every 1 second that submits to a channel
	go generateEvents(c)
...
func statsHandler(w http.ResponseWriter, r *http.Request) {
	...
	cvs := make([]ClientView, len(hub.GetClients()))

	for i, cl := range hub.GetClients() {
		cvs[i] = ClientView{IpAddress: cl.Ip}

	}
	//w.Write([]byte(fmt.Sprintf("Number of clients %d", len(hub.GetClients()))))
